...
function generateDemoContributions(weeks: number): Contribution[] {
  ...
  return Array.from({ length: weeks * 7 }, (_, i) => {
    ...
    // Create a natural contribution pattern
...
const RepoRow = ({
  ...
  const content = (
    <>
      ...
      <span className="text-sm tabular-nums text-foreground/70 font-mono">
        {repo.count}
      </span>
    </>
  );
...
const DEMO_REPOS: RepoContribution[] = [
  { name: 'amicro-ui', count: 142 },
  { name: 'evilcharts', count: 98 },
  { name: 'monocharts', count: 64 },
];
...
export function GitHubActivity({
  ...
  const demoContributions = React.useMemo(() => generateDemoContributions(weeksFor(months)), [months]);

  const needsFetch = username && (!contributionsProp?.length || !reposProp?.length);
  const fetched = useGitHubUser(needsFetch ? username : undefined);

  const contributions = contributionsProp?.length
    ? contributionsProp
    : (fetched?.contributions ?? demoContributions);
  const repos = reposProp?.length
    ? reposProp
    : (fetched?.repos?.length ? fetched.repos : DEMO_REPOS);
  ...
  return (
    <div
      ...
      className={cn(
        "relative max-w-full overflow-hidden rounded-[24px] transition-all duration-300 group flex flex-col justify-between p-4 sm:p-5 font-sans",
        isDark
    ...
    >
      <div className="flex items-center justify-between mb-3 px-1">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-mono bg-emerald-500/20 text-emerald-400 border border-emerald-500/30">
              GitHub Heatmap
            </span>
          </div>
          <p className="text-lg font-bold tracking-tight mt-0.5 font-sans">
            {heading}
          </p>
        </div>
      </div>
      ...
      {repos.length > 0 && (
        <motion.div
          ...
          className={cn(
            "absolute inset-x-3 bottom-3 overflow-hidden backdrop-blur-xl border transition-colors",
            isDark
              ? "bg-[#131313]/90 border-white/10 text-white"
              : "bg-neutral-100/90 border-neutral-200 text-black",
            open && "top-3 z-20",
          )}
          ...
          >
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className={`grid size-7 shrink-0 place-items-center rounded-full transition-colors cursor-pointer ${
